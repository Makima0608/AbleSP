...
void UAblePlayAnimationTask::CollectDependencies(UAbleAbilityContext* Context, TArray<FString>& OutDeps)
{
	Super::CollectDependencies(Context, OutDeps);
	CollectDependenciesBP(Context, OutDeps);
}

void UAblePlayAnimationTask::CollectDependenciesBP_Implementation(UAbleAbilityContext* Context,
	TArray<FString>& OutDeps)
{
	const auto AnimationPath = GetSoftAnimationAsset().ToString();
	if (Context->GetSelfAbilityComponent())
	{
		const auto AdaptedAnimationPath = Context->GetSelfAbilityComponent()->ProcessAnimationFromBodyType(AnimationPath);
		if (!AdaptedAnimationPath.IsEmpty() && (AdaptedAnimationPath != AnimationPath) && FPackageName::DoesPackageExist(AdaptedAnimationPath))
		{
			OutDeps.AddUnique(AdaptedAnimationPath);
			return;
		}
	}
}
...
void UAblePlayAnimationTask::OnTaskStartBP_Implementation(const UAbleAbilityContext* Context) const
{
	...
	const UAnimationAsset* AnimationAsset = m_AnimationAsset;
...
FText UAblePlayAnimationTask::GetRichTextTaskSummary() const
{
	...
	{
		AnimationName = FString::Format(TEXT("<a id=\"AbleTextDecorators.AssetReference\" style=\"RichText.Hyperlink\" PropertyName=\"m_AnimationAsset\" Filter=\"AnimationAsset\">{0}</>"), { !GetSoftAnimationAsset().ToString().IsEmpty() ? GetSoftAnimationAsset().ToString() : AnimationName });
	}
